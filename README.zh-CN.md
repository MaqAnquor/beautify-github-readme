...
| 只生成 SVG 装饰 | 生成首图、章节标题、流程图、徽章或一组视觉模块，并提供本地预览 | 不改 README 正文、顺序、图片引用或链接 |

如果请求已经说明范围，Skill 会直接执行。只说“美化这个仓库”或只提供仓库地址时，Agent 会先问：

```text
这次希望我优化整份 README，还是只生成 SVG 装饰？
如果只做 SVG，请告诉我是首图、章节标题、流程图、徽章，还是一组视觉模块。
```
...
```text
[$beautify-github-readme] 保留 README 不动，只生成一张 SVG 首图和三张章节标题。
根据项目现有风格设计，先给我渲染预览。
```

SVG-only 模式下，即使 Agent 为了理解项目读取了 README，也不代表可以修改它。需要把新素材嵌入 README 时，会再次取得明确授权。
